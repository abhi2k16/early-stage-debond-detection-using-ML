...
X_centre = zeros(15002,25);
for i = 1:5
    for j = 1:5
        dam_cat = 'U2_at_centre_';
        file = append(num2str(8+(i-1)*2),'x',num2str(8+(j-1)*2),'mm',num2str(150),'ms','.','rpt');
        ...
        X_centre(1:15001,(i-1)*5+j) = x;
    end
end
X_centre(15002,1:25)=D_area_cen;
%xlswrite('U2_at_centre_8x8_to_16x16_150ms.xlsx',X_centre);
%% data collection for edge
X_edge = zeros(15002,21);
for i = 1:7
    for j = 1:3
        dam_cat = 'U2_at_edge_';
        file = append(num2str(8+(i-1)*2),'x',num2str(10+(j-1)*1),'mm',num2str(150),'ms','.','rpt');
        D_area_edg((i-1)*3+j) = (8+(i-1)*2)*(10+(j-1)*1);
        file_name= append('C:\Users\abhij\Desktop\WaveModel\PZT_Model\Dam_Category\',dam_cat,file);
        x_e = importdata(file_name).data(:,2);
        X_edge(1:15001,(i-1)*3+j) = x_e;
    end
end
X_edge(15002,1:21)=D_area_edg;
%xlswrite('U2_at_edge_8x10_to_20x12_150ms.xlsx',X_edge);
%% data collection for centre_edge
X_centre_edge = zeros(15002,20);
for i = 1:5
    for j = 1:4
        dam_cat = 'U2_at_centre_edge_';
        file = append(num2str(8+(i-1)*2),'x',num2str(15+(j-1)*1),'mm',num2str(150),'ms','.','rpt');
        D_area_cen_edg((i-1)*4+j) = (8+(i-1)*2)*(15+(j-1)*1);
        file_name= append('C:\Users\abhij\Desktop\WaveModel\PZT_Model\Dam_Category\',dam_cat,file);
        x_e = importdata(file_name).data(:,2);
        X_centre_edge(1:15001,(i-1)*4+j) = x_e;
    end
end
X_centre_edge(15002,1:20)=D_area_cen_edg;
%xlswrite('U2_at_centre_edge_8x15_to_16x18_150ms.xlsx',X_centre_edge);
%% data collection for through_the_width
X_thr_width = zeros(15002,9);
for i = 1:9
    for j = 1:1
        dam_cat = 'U2_thr_width_';
        file = append(num2str(8+(i-1)*1),'x',num2str(26+(j-1)*1),'mm',num2str(150),'ms','.','rpt');
        D_area_thr_wid((i-1)*1+j) = (8+(i-1)*1)*(26+(j-1)*1);
        file_name= append('C:\Users\abhij\Desktop\WaveModel\PZT_Model\Dam_Category\',dam_cat,file);
        x_e = importdata(file_name).data(:,2);
        X_thr_width(1:15001,(i-1)*1+j) = x_e;
    end
end
X_thr_width(15002,1:9) = D_area_thr_wid;
xlswrite('U2_thr_width_8x26_to_16x26_150ms.xlsx',X_thr_width);
